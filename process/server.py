...
#Inport EmailDoorAlarm.conf
config = {}

file_name = "/home/pi/EmailDoorAlarm.conf"
...
# logging
logging.basicConfig(filename=config["python_logs_path"].strip('"\"') ,level=logging.DEBUG,format='%(asctime)s - (%(threadName)-9s) - %(message)s',)
...
f = open(config["path_email_alarm_status"].strip('"\''),"r")
email_alarm_status = f.read().rstrip('\n')
...
event_logging_status = config["event_timeline_status"].strip('"\"')

#timer
global countdown_notify_status
countdown_notify_status = config["countdown_notify_status"].strip('"\"')
...
def status(argument):
    ...
    if not email_alarm_status:
        with open(config["path_email_alarm_status"].strip('"\'')) as fp:
            email_alarm_status = fp.read()
        with open(config["path_email_alarm_status2"].strip('"\''),"w") as cp:
            cp.write(email_alarm_status)

    with open(config["path_email_alarm_status"].strip('"\'')) as fp:
        with open(config["path_email_alarm_status2"].strip('"\'')) as cp:
            if cp.read() != fp.read():
                email_alarm_status = fp.read()
                print "changed"
...
def log_event(door):
    ...
    if os.path.isfile(config["json_events_data"].strip('"\'')):
    ...
    if door:
        ...
        if os.path.isfile(config["json_events_data"].strip('"\'')) and ids >= 1:
            ...
            print datain
        elif os.path.isfile(config["json_events_data"].strip('"\'')) and ids == 0:
...
def sendmail():
    fromaddr = config["Sender"].strip('"\'')
    toaddr = config["To_Email_Address"].strip('"\'')
    msg = MIMEMultipart()
    msg['From'] = fromaddr
    msg['To'] = toaddr
    ...
    server.starttls()
    server.login(config["login"].strip('"\''), config["pwd"].strip('"\''))
    text = msg.as_string()
...
logging.debug("Email alert is: %s", (email_alarm_status))
...
if __name__ == '__main__':
    ...
    wm.add_watch(config["path_email_alarm_status"].strip('"\''), pyinotify.IN_MODIFY)
